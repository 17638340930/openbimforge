...
# Compatibility shim for transformers.tools
try:
    ...
    # Try newer 'agents' path if 'tools' is missing
    try:
        from transformers.agents import Agent
        from transformers.agents.agent_types import Agent as AgentType
        # Basic stubs for missing legacy functions
        resolve_tools = lambda x, *args, **kwargs: x
        get_tool_creation_code = lambda *args, **kwargs: ""
        BASE_PYTHON_TOOLS = {}
    except (ImportError, ModuleNotFoundError):
        # Fallback to a minimal stub if agents are totally missing
        class Agent: pass
        resolve_tools = lambda x, *args, **kwargs: x
        get_tool_creation_code = lambda *args, **kwargs: ""
        BASE_PYTHON_TOOLS = {}
...
from tool_agent.runtime_config import (
    PROJECT_ROOT,
    get_runtime_workflow_model,
    get_unified_llm_config,
)
...
try:
    ...
    genai = None
    # Provide stubs for types used in type hints if necessary
    FunctionDeclaration = GenerateContentConfig = HttpOptions = Tool = ToolConfig = \
    FunctionCallingConfig = Content = Part = ThinkingConfig = Any

# set the project root path   JY鏉╂瑩鍣风憰浣规暭
...
ARCHITECT_PROMPT_PATH = os.path.join(PROJECT_ROOT, "forge_core", "design_agent", "muti_agent_prompt", "floor_plan_designer_chat_prompt_few_shots.txt")
...
VERTEX_PROJECT = os.environ.get("VERTEX_PROJECT", "")  # Your Google Cloud project ID
# VERTEX_LOCATION = "us-central1"  # Your Google Cloud region
VERTEX_LOCATION = os.environ.get("VERTEX_LOCATION", "global")  # Your Google Cloud region
DEEPSEEK_API_KEY = os.environ.get("DEEPSEEK_API_KEY", "")
RUNTIME_LLM_CONFIG = get_unified_llm_config()
DEFAULT_WORKFLOW_MODEL = get_runtime_workflow_model()


def get_runtime_provider() -> str:
    return RUNTIME_LLM_CONFIG.get("provider", "")


def get_runtime_model_id(default_model: str) -> str:
    return RUNTIME_LLM_CONFIG.get("modelId") or default_model


def get_runtime_api_key(env_name: str, fallback: str = "") -> str:
    if RUNTIME_LLM_CONFIG.get("apiKey"):
        return RUNTIME_LLM_CONFIG["apiKey"]
    return os.environ.get(env_name, fallback)


def get_runtime_base_url(default_base_url: str = "") -> str:
    return RUNTIME_LLM_CONFIG.get("baseUrl") or default_base_url
...
def plan_designer(query: str, 
    ...
    # Use the runtime model if available
    from tool_agent.runtime_config import get_unified_llm_config
    llm_config = get_unified_llm_config()
    actual_model = model
    if actual_model == "openai" and llm_config.get("modelId"):
        actual_model = llm_config["modelId"]

    if "gpt" in actual_model or "o1" in actual_model or "o4" in actual_model:
        client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY", OPENAI_API_KEY))
        messages = [{"role": "user","content": str(prompt)}]
        model_name = actual_model if "gpt" in actual_model else "o4-mini-2025-04-16"
        second_response = client.chat.completions.create(
            model=model_name,
            messages=messages,
            temperature=1 if "o1" in model_name or "o4" in model_name else 0,
        )
    ...
    if "gemini" in actual_model:
        ...
        client = genai.Client(vertexai=True, project=VERTEX_PROJECT, location=VERTEX_LOCATION, http_options=HttpOptions(api_version='v1'))
        second_response = client.models.generate_content(
            model="gemini-2.5-pro-preview-05-06",
            contents=prompt,
            config=GenerateContentConfig(temperature=0.1, stop_sequences=["User:"], max_output_tokens=65535),
        )
        return second_response.text
    
    if "mistral" in actual_model:
        api_key = os.environ.get("MISTRAL_API_KEY", MISTRAL_API_KEY)
        if not api_key: return "Error: Mistral API key missing."
        client = Mistral(api_key=api_key)
        chat_response = client.chat.complete(model="mistral-medium-2505", messages=[{"role": "user", "content": prompt}], temperature=0)
    ...
    if "claude" in actual_model:
        ...
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": prompt}], stop_sequences=["User:"]
        )
        return response.content[0].text

    # Fallback for ANY OpenAI compatible provider (LongCat, DeepSeek, etc.)
    ...
    api_key = (
        ...
        or ("ollama" if provider == "ollama" or "11434" in str(base_url or "") else "")
    )
    ...
    client_kwargs = {
        ...
        "timeout": httpx.Timeout(1800.0, connect=60.0, read=600.0, write=60.0),
        "http_client": httpx.Client(
            verify=False,
            trust_env=False,  # Bypass system proxies that cause SSL EOF
            http2=False,
            headers={"User-Agent": "openBIMForge-Nexus-Runtime/1.0 (BIM Automation)"}
        )
    }
    if base_url:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_error = e
            wait = 2 ** attempt
            print(f"\n[plan_designer Retry {attempt}/{max_retries}] {e}")
            if attempt < max_retries:
                print(f"[plan_designer] Waiting {wait}s before retry...")
                time.sleep(wait)
                # Recreate httpx client for fresh TLS connection
...
class MyAgent(Agent):
    ...
    def set_stream(self, streamer: Callable[[str], None]):
        """Set the streamer function for real-time output."""
        self.stream = streamer

    def log(self, msg: str):
        """Log message and send to stream if available."""
        print(msg)
        if self.stream:
            self.stream(msg)

    def prepare_for_new_chat(self):
        """Initialize or reset the chat state for a new session."""
        self.chat_state = {}
    ...
    def chat_with_function_call(self, task:str, chat_history:str, stop=["User:","Programmer:","Product Owner:"]):
        ...
        # ===== 閺傛澘顤?DeepSeek 閸掑棙鏁?=====   JY
        if isinstance(self, DeepSeekAgent):
...
class OpenAiAgent(MyAgent):
    def __init__(
        ...
        if api_key is None:
            print("[WARNING] OpenAI API key not found. Agent will fail if used.")
    ...
    def _chat_generate(self, prompt, stop):
        ...
        if "o1" in self.model or "o4" in self.model:
...
class DeepSeekAgent(MyAgent):
    def __init__(
        ...
        try:
            ...
            raise ImportError("Using `DeepSeekAgent` requires `openai`: `pip install openai`.")
        ...
        if api_key is None:
            if "DEEPSEEK_API_KEY" in globals():
                api_key = globals().get("DEEPSEEK_API_KEY")

        if api_key is None or api_key == "":
            # Don't raise error at init, only warn.
            print("[WARNING] DeepSeek API key not found. Agent will fail if used.")
    ...
    def generate_with_img(self, prompt, stop, img_path):
        """Generate response with image input using DeepSeek's vision capabilities."""
        data_url = local_image_to_data_url(img_path)
        messages = [{
            "role": "user", 
            "content": [
    ...
    def _chat_generate(self, prompt, stop):
        """Internal method for generating chat completions."""
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[{"role": "user", "content": prompt}],
            temperature=0,
