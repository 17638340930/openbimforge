...
export function getAIModel(overrides?: ClientOverrides): ModelConfig {
    ...
    if (!modelId) {
        if (hasClientConfig) {
            throw new Error(
                `No model selected in the front-end settings. Please choose a main generation model in "API Keys & Models".`,
            )
        }
        throw new Error(
            `No model configured. Either select a model in the front-end settings or set AI_MODEL in the environment.`,
        )
    }
    ...
    switch (provider) {
        ...
        case "ollama": {
            let baseURL = overrides?.baseUrl || process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434/v1"
            
            // Normalize baseURL for OpenAI compatibility: Ollama's OpenAI endpoint is at /v1
            if (!baseURL.includes("/v1") && !baseURL.includes("/api")) {
                baseURL = baseURL.replace(/\/$/, "") + "/v1"
            }

            const apiKey = overrides?.baseUrl
                ? overrides?.apiKey || "ollama"
                : resolveApiKey(overrides, "OLLAMA_API_KEY") || "ollama"

            // Use createOpenAI for Ollama - much more stable path resolution
...
/**
 * Get the AI model for BIM validation.
