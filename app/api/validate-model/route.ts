    ...
    try {
        ...
        switch (provider) {
            ...
            case "ollama": {
                ...
                let finalBaseUrl = baseUrl || process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434"
                if (!finalBaseUrl.includes("/v1") && !finalBaseUrl.includes("/api")) {
                    finalBaseUrl = finalBaseUrl.replace(/\/$/, "") + "/v1"
                }

                // Use createOpenAI for Ollama as it's more stable for OpenAI-compatible endpoints
                const openai = createOpenAI({
