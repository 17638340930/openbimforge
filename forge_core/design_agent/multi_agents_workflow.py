...
from tool_agent.agents import ClaudeAgent, OpenAiAgent, GeminiAgent, MistralAgent, DeepSeekAgent, PROJECT_ROOT
...
PRODUCT_OWNER_PROMPT_PATH = os.path.join(PROJECT_ROOT, "forge_core", "design_agent", "muti_agent_prompt", "po_chat_prompt_temp.txt")
CODER_PROMPT_PATH = os.path.join(PROJECT_ROOT, "forge_core", "design_agent", "muti_agent_prompt", "coder_chat_prompt_temp.txt")
REVIEWER_PROMPT_PATH = os.path.join(PROJECT_ROOT, "forge_core", "design_agent", "muti_agent_prompt", "checker_chat_prompt_temp.txt")
...
MODEL = "deepseek" #JY这里是修改模型为deepseek
...
def run_po_coder_agents(query: str, 
    ...
    elif model == "deepseek":  # JY添加 deepseek 分支
        agent_po = DeepSeekAgent(
            model="deepseek-chat",  # 或者 "deepseek-coder"
            chat_prompt_template=po_prompt_str,
            additional_tools=tool_list
        )
        agent_coder = DeepSeekAgent(
            model="deepseek-chat",  # 或者 "deepseek-coder"
            chat_prompt_template=coder_prompt_str,
            additional_tools=tool_list
        )
