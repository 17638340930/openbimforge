...
- **核心架构设计**：自主研发 openBIMForge 自动化建模系统，采用多 Agent（Architect/Constructor）编排架构，实现了自然语言需求到专业 BIM 软件（Vectorworks）的端到端生成闭环。
- **意图对齐优化**：设计并实现槽位填充式意图澄清环（Clarification Loop），有效解决了 LLM 在专业建筑建模任务中的意图模糊问题，显著降低了无效推理成本。
- **幻觉抑制工程**：提出“工具契约（Tool Contract）”机制，通过实时扫描宿主软件能力集进行生成约束，将构件名称与参数的生成准确率提升至工业应用水平。
