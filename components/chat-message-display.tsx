...
function sanitizeDisplayText(text: string): string {
    ...
    const withForgeVisionPlaceholder = text
        .replace(
            /(?:\u3010ForgeVisionConstraints\u3011|ã€ForgeVisionConstraintsã€‘|Ã£â‚¬ÂForgeVisionConstraintsÃ£â‚¬â€˜)[\s\S]*?(?=\u3010用户补充需求\u3011|ã€ç”¨æˆ·è¡¥å……éœ€æ±‚ã€‘|Constraint policy:|$)/g,
            "📦 [已载入 ForgeVision 几何约束供主模型参考]\n\n",
        )
        .replace(
            /```json\s*([\s\S]*?(?:"source"\s*:\s*"forgevision-form"|forgeVisionConstraints|cadVectorPaths)[\s\S]*?)```/gi,
            "📦 [已载入 ForgeVision 几何约束供主模型参考]",
        )
        .replace(
...
function isForgeVisionConstraintText(text: string): boolean {
    return (
        ...
        text.includes("Ã£â‚¬ÂForgeVisionConstraintsÃ£â‚¬â€˜") ||
        (/ForgeVision/i.test(text) && /cadVectorPath/i.test(text))
    )
}
...
function StageDetailItem({ stage }: { stage: ExecutionStage }) {
    ...
    return (
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-zinc-50/80 group">
            ...
            <div className="min-w-0 flex-1">
                ...
                {stage.detail && (
                    <p className="mt-0.5 truncate text-[11px] text-zinc-400 transition-all group-hover:whitespace-normal group-hover:break-words">
                        {stage.detail}
                    </p>
                )}
            </div>
...
export function ExecutionCard({ payload }: { payload: ExecutionLogPayload }) {
    ...
    const isForgeVision = payload.mode === "ForgeVision-Form"
    ...
    return (
        <div className="my-4 max-w-[420px] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition-all hover:shadow-md">
            <div
                ...
                <div className="flex items-center gap-3">
                    <div
                        className={cn(
                            ...
                            overallStatus === "success" ? "bg-emerald-500" : overallStatus === "failed" ? "bg-rose-500" : "animate-pulse bg-blue-500",
                        )}
                    />
                    <span className="text-[13px] font-bold tracking-tight text-zinc-800">
                        {isForgeVision ? "ForgeVision-Form 形体转化" : "NEXUS BIM 协同编排流"}
                    </span>
                    <span
                        className={cn(
                    ...
                    >
                        {overallStatus === "success" ? `${completedStages}/${totalStages} 阶段已完成` :
                            overallStatus === "failed" ? "构筑失败" :
                                hasVmWaiting ? `${completedStages}/${totalStages} 等待 VM 回写...` :
            ...
            {expanded && (
                <div className="space-y-0.5 border-t border-zinc-100 bg-white px-3 py-2">
                    {displayStages.map((stage, i) => <StageDetailItem key={stage.id || i} stage={stage} />)}
                </div>
            )}
                ...
                <div className="border-t border-zinc-100 bg-emerald-50/40 px-4 py-3">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            {isForgeVision ? (
                                <>
                                    <h4 className="text-[13px] font-bold leading-tight text-emerald-900">图片形体参考已完成</h4>
                                    <div className="mt-2 space-y-1 text-[12px] leading-relaxed text-emerald-800/90">
                                        <p><span className="font-semibold">执行状态：</span>{payload.summary || "ForgeVision-Form 已返回参考产物。"}</p>
                                        <p><span className="font-semibold">下一步：</span>系统将自动进入 Nexus 编排；如未触发，可点击“采用图片形体继续生成 BIM”。</p>
                                    </div>
                                </>
                            ) : (
                                <>
                                    <h4 className="text-[13px] font-bold leading-tight text-emerald-900">BIM 模型已成功注入 Vectorworks</h4>
                                    <div className="mt-2 space-y-1 text-[12px] leading-relaxed text-emerald-800/90">
